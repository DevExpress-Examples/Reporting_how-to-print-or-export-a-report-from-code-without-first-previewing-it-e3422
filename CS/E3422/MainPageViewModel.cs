...
namespace E3422 {
    public class MainPageViewModel : INotifyPropertyChanged {
        ...
        IReportServiceClient CreateClient() {
            ReportServiceClientFactory factory = new ReportServiceClientFactory();
            return factory.Create(new EndpointAddress(ReportServiceUri));
        }

        ParameterStub[] CreateParameters() {
            return new ParameterStub[] {
                new ParameterStub() { Path = "stringParameter", Value = ParameterValue  }
            };
        }
